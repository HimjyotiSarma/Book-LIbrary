...
class Book {
  ...
  addBookToLibrary() {
    ...
    Book.BookLibrary.push(this)
    console.log(Book.BookLibrary)
    localStorage.setItem('Book_Library', JSON.stringify(Book.BookLibrary))
  }
}
...
dialog.addEventListener('click', (e) => {
  ...
  if (
    e.clientX < dialogDimension.left - 1 ||
    e.clientX > dialogDimension.right + 1 ||
    e.clientY < dialogDimension.top - 1 ||
    e.clientY > dialogDimension.bottom + 1
  ) {
    dialog.close()
  }
})
// Get Form Data and Form Action on Successful data get
...
// Form Action on Successful Data retrieval
